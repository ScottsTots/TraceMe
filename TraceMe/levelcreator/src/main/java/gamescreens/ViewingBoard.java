...
public class ViewingBoard extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        */
      //  Log.d("viewingBoard", " viewing drawwwwwing now");
        if(GameActivity.traceBitmaps.size() > 0) {
            if(GameActivity.additiveMode) {
                mCanvas.drawBitmap(GameActivity.traceBitmaps.get(currentBitmap),0,0,mBitmapPaint);
                canvas.drawBitmap(mBitmap,0,0,mBitmapPaint);
            } else {
                canvas.drawBitmap(GameActivity.traceBitmaps.get(currentBitmap), 0, 0, mBitmapPaint);
            }
            if (System.currentTimeMillis() - prevTime > 1000) {
                if (currentBitmap + 1 < GameActivity.traceBitmaps.size()) {
                    currentBitmap++;
                } else {
