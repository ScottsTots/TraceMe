...
public class DrawingBoard extends View {
    ...
    private Bitmap mBitmapAdditive;
    private Canvas mCanvas;
    private Canvas mCanvasAdditive;
    private Path mPath;
    ...
    public DrawingBoard(Context c, AttributeSet attributeSet) {
      ...
      //    mPaint.setStrokeCap(Paint.Cap.ROUND);
        ...
        mBitmapAdditive = Bitmap.createBitmap(frameBufferWidth, frameBufferHeight, Bitmap.Config.ARGB_8888);
    ...
    private void touch_up() {
        ...
        if(GameActivity.additiveMode) { // if in additive mode, everytime we touch up we save a new trace
            mBitmapAdditive.eraseColor(Color.TRANSPARENT);
            mCanvas.setBitmap(mBitmapAdditive);
            mCanvas.drawPath(mPath, mPaint);
            GameActivity.traceBitmaps.add( mBitmapAdditive.copy(Bitmap.Config.ARGB_8888, true));
            // TODO. add path length before we reset it.

            TraceFile file = new TraceFile( mBitmapAdditive.copy(Bitmap.Config.ARGB_8888, true), new ArrayList<DataPoint>(GameActivity.pointsArray));
            GameActivity.traceArray.add(file);
           // Log.d("additivemode", " updating numtraces to " + GameActivity.traceArray.size());

            GameActivity.pathsArray.clear();
            GameActivity.pointsArray.clear(); // clear current point data.
            GameActivity.updateNumTraces();
            mCanvas.setBitmap(mBitmap); // go back to normal bitmap.
        }
